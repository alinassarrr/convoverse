...
export async function GET() {
  try {
    ...
    const n8nGmailWebHook = 'http://localhost:5678/webhook/4ad0cbb1-1db7-4323-940d-0bf77430cdc2';
    ...
    const responseData = response.ok ? await response.text().catch(() => 'OK') : null;
    
    console.log('Gmail sync webhook response:', response.status, response.statusText);
    ...
    if (error.cause?.code === 'ECONNREFUSED') {
      return NextResponse.json(
        { message: 'n8n service is not available. Please check if n8n is running on localhost:5678' },
        { status: 503 }
      );
    }
